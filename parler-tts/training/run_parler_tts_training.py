    ...
    if data_args.save_to_disk is not None and not dataset_was_precomputed:
        if accelerator.is_main_process:
            vectorized_datasets.save_to_disk(
                ...
                # num_proc = min(data_args.preprocessing_num_workers, len(vectorized_datasets["eval"]) - 1) if data_args.do_eval_custom and "eval" in vectorized_datasets else data_args.preprocessing_num_workers
                # num_proc = min(data_args.preprocessing_num_workers, len(vectorized_datasets["eval"]) - 1) if data_args.do_eval and "eval" in vectorized_datasets else data_args.preprocessing_num_workers
                num_proc = data_args.preprocessing_num_workers
            )
        accelerator.wait_for_everyone()
